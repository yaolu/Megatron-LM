...
from correct_retrieved_instance_idx import quac_correct_retrieved_instance_idx_list
import copy
import re
...
def evaluate_f1(ground_truth_file, prediction_file, reduced_test_only=False):
    ...
    if "inscit" in ground_truth_file:
        ...
        for answers in groundtruth_answers:
            answers_update = []
            for ans in answers:
                ## this answer is additionally added to the answer_list, needs to remove
...
def evaluate_f1_llmware(ground_truth_file, prediction_file1, prediction_file2):
    predicted_answers1 = load_prediction(prediction_file1)
    predicted_answers2 = load_prediction(prediction_file2)

    unanswerable_acc = 0
    unanswerable_idx = 0

    final_predicted_answers = []
    groundtruth_answers = []
    with open(ground_truth_file, "r") as f:
        gold_data_list = json.load(f)
    for idx, item in enumerate(gold_data_list):
        groundtruth_answers.append([item['answer']])
        if item['category'] == 'not_found_classification':
            final_predicted_answers.append(predicted_answers2[unanswerable_idx])
            if predicted_answers2[unanswerable_idx] == "Sorry. I cannot find the answer based on the context.":
                unanswerable_acc += 1
            unanswerable_idx += 1
        else:
            final_predicted_answers.append(predicted_answers1[idx])

    compute_f1_score(final_predicted_answers, groundtruth_answers)
    # print("number of unanswerable cases:", unanswerable_idx)
    print("unanswerable accuracy:", unanswerable_acc/unanswerable_idx)
...
def separate_cannot_answer(ground_truth_file, prediction_file, topk=5, is_doqa=False):
    ...
    if "quac" in prediction_file:
        print("getting answerable_instance_idx_list for quac")
    ...
    for idx, item in enumerate(groundtruth_answers):
        if is_doqa:
        ...
        elif "topiocqa" in prediction_file:
            ...
            if not all_NOANSWER and final_answer == "Sorry. I cannot find the answer based on the context.":
                ## remove the case where annotators do not give consistent unanswerable answers
                continue
...
if __name__ == "__main__":
    ...
    ## doc2dial
    prediction_file = ckpt_path + "/doc2dial_{}_generate_70b_test_greedy_0_4000_3438_ret.txt.v2".format(n_ctx)
    # prediction_file = ckpt_path + "/doc2dial_dragon_{}_generate_70b_test_greedy_0_4000_3438_ret.txt.v2".format(n_ctx)
    # prediction_file = ckpt_path + "/doc2dial_{}_generate_70b_test_greedy_0_4000_3438_ret.txt.v2.reverse_ctxs".format(n_ctx)
    # prediction_file = ckpt_path + "/doc2dial_{}_generate_70b_test_greedy_0_4000_3438_ret.txt.v2.swing_ctxs".format(n_ctx)
    # prediction_file = ckpt_path + "/doc2dial_{}_generate_70b_test_greedy_0_4000_3438_ret.txt.v2.shuffle_ctxs".format(n_ctx)
    # prediction_file = "/lustre/fsw/adlr/adlr-nlp/zihanl/datasets/foundational-qa/multi-turn-qa/doc2dial/chatgpt_3.5_turbo_based_on_ftdragon_chatgptgen7k_chunk150.txt"
    # prediction_file = "/lustre/fsw/adlr/adlr-nlp/zihanl/datasets/foundational-qa/multi-turn-qa/doc2dial/gpt_4_based_on_ftdragon_chatgptgen7k_chunk150_final.txt"
    ...
    ## quac
    prediction_file = ckpt_path + "/quac_{}_generate_70b_test_greedy_0_8000_3438_ret.txt.v2".format(n_ctx)
    # prediction_file = ckpt_path + "/quac_dragon_{}_generate_70b_test_greedy_0_8000_3438_ret.txt.v2".format(n_ctx)
    # prediction_file = ckpt_path + "/quac_{}_generate_70b_test_greedy_0_8000_3438_ret.txt.v2.reverse_ctxs".format(n_ctx)
    # prediction_file = ckpt_path + "/quac_{}_generate_70b_test_greedy_0_8000_3438_ret.txt.v2.swing_ctxs".format(n_ctx)
    # prediction_file = ckpt_path + "/quac_{}_generate_70b_test_greedy_0_8000_3438_ret.txt.v2.shuffle_ctxs".format(n_ctx)
    # prediction_file = "/lustre/fsw/adlr/adlr-nlp/zihanl/datasets/foundational-qa/multi-turn-qa/quac/chatgpt_3.5_turbo_based_on_ftdragon_chatgptgen7k_chunk150.txt"
    # prediction_file = "/lustre/fsw/adlr/adlr-nlp/zihanl/datasets/foundational-qa/multi-turn-qa/quac/gpt_4_based_on_ftdragon_chatgptgen7k_chunk150_final.txt"
    ...
    ## qrecc
    prediction_file = ckpt_path + "/qrecc_{}_generate_70b_test_greedy_0_4000_3438_ret.txt.v2".format(n_ctx)
    # prediction_file = ckpt_path + "/qrecc_dragon_{}_generate_70b_test_greedy_0_4000_3438_ret.txt.v2".format(n_ctx)
    # prediction_file = ckpt_path + "/qrecc_{}_generate_70b_test_greedy_0_4000_3438_ret.txt.v2.reverse_ctxs".format(n_ctx)
    # prediction_file = ckpt_path + "/qrecc_{}_generate_70b_test_greedy_0_4000_3438_ret.txt.v2.swing_ctxs".format(n_ctx)
    # prediction_file = ckpt_path + "/qrecc_{}_generate_70b_test_greedy_0_4000_3438_ret.txt.v2.shuffle_ctxs".format(n_ctx)
    # prediction_file = "/lustre/fsw/adlr/adlr-nlp/zihanl/datasets/foundational-qa/multi-turn-qa/qrecc/chatgpt_3.5_turbo_based_on_ftdragon_chatgptgen7k_chunk150.txt"
    # prediction_file = "/lustre/fsw/adlr/adlr-nlp/zihanl/datasets/foundational-qa/multi-turn-qa/qrecc/gpt_4_based_on_ftdragon_chatgptgen7k_chunk150_without_shuffle_final.txt"
    # prediction_file = "/lustre/fsw/adlr/adlr-nlp/zihanl/datasets/foundational-qa/multi-turn-qa/qrecc/gpt_4_based_on_ftdragon_chatgptgen7k_chunk150.txt"
    ...
    ## coqa
    prediction_file = ckpt_path + "/coqa_{}_generate_70b_test_greedy_0_8000_3438_ret.txt.v2".format(n_ctx)
    # prediction_file = "/lustre/fsw/adlr/adlr-nlp/zihanl/datasets/foundational-qa/multi-turn-qa/coqa/chatgpt_3.5_turbo_gen.txt"
    # prediction_file = "/lustre/fsw/adlr/adlr-nlp/zihanl/datasets/foundational-qa/multi-turn-qa/coqa/gpt_4_based_on_ftdragon_chatgptgen7k_chunk150_coqa_final.txt"
    ...
    ## doqa_cooking
    prediction_file = ckpt_path + "/doqa_cooking_{}_generate_70b_test_greedy_0_2000_3438_ret.txt.v2".format(n_ctx)
    # prediction_file = "/lustre/fsw/adlr/adlr-nlp/zihanl/datasets/foundational-qa/multi-turn-qa/doqa/chatgpt_3.5_turbo_gen_doqa_cooking.txt"
    # prediction_file = "/lustre/fsw/adlr/adlr-nlp/zihanl/datasets/foundational-qa/multi-turn-qa/doqa/gpt_4_based_on_ftdragon_chatgptgen7k_chunk150_doqa_cooking_final.txt"
    ...
    # doqa_travel
    prediction_file = ckpt_path + "/doqa_travel_{}_generate_70b_test_greedy_0_2000_3438_ret.txt.v2".format(n_ctx)
    # prediction_file = "/lustre/fsw/adlr/adlr-nlp/zihanl/datasets/foundational-qa/multi-turn-qa/doqa/chatgpt_3.5_turbo_gen_doqa_travel.txt"
    # prediction_file = "/lustre/fsw/adlr/adlr-nlp/zihanl/datasets/foundational-qa/multi-turn-qa/doqa/gpt_4_based_on_ftdragon_chatgptgen7k_chunk150_doqa_travel_final.txt"
    ...
    ## doqa_movies
    prediction_file = ckpt_path + "/doqa_movies_{}_generate_70b_test_greedy_0_2000_3438_ret.txt.v2".format(n_ctx)
    # prediction_file = "/lustre/fsw/adlr/adlr-nlp/zihanl/datasets/foundational-qa/multi-turn-qa/doqa/chatgpt_3.5_turbo_gen_doqa_movies.txt"
    # prediction_file = "/lustre/fsw/adlr/adlr-nlp/zihanl/datasets/foundational-qa/multi-turn-qa/doqa/gpt_4_based_on_ftdragon_chatgptgen7k_chunk150_doqa_movies_final.txt"
    ...
    ## convfinqa
    prediction_file = ckpt_path + "/convfinqav3_{}_generate_70b_test_greedy_0_1500_3438_ret.txt.v2".format(n_ctx)
    # prediction_file = "/lustre/fsw/adlr/adlr-nlp/zihanl/datasets/foundational-qa/multi-turn-qa/convfinqav3/chatgpt_3.5_turbo_gen_convfinqav3.txt"
    # prediction_file = "/lustre/fsw/adlr/adlr-nlp/zihanl/datasets/foundational-qa/multi-turn-qa/convfinqav3/gpt_4_convfinqa_final.txt"
    ...
    # evaluate_finqa_and_convfinqa_chatgpt(ground_truth_file, prediction_file)

    # ## finqa
    # prediction_file = ckpt_path + "/finqav2_{}_generate_70b_test_greedy_0_1500_3438_ret.txt.v2".format(n_ctx)
    ...
    ## sqa
    prediction_file = ckpt_path + "/sqa_{}_generate_70b_test_greedy_0_3100_3438_ret.txt.v2".format(n_ctx)
    # prediction_file = "/lustre/fsw/adlr/adlr-nlp/zihanl/datasets/foundational-qa/multi-turn-qa/sqa/chatgpt_3.5_turbo_gen_sqa.txt"
    # prediction_file = "/lustre/fsw/adlr/adlr-nlp/zihanl/datasets/foundational-qa/multi-turn-qa/sqa/gpt_4_sqa_final.txt"
    ...
    ## topiocqa
    prediction_file = ckpt_path + "/topiocqa_20_generate_70b_test_greedy_0_2600_3438_ret.txt.v2"
    # prediction_file = ckpt_path + "/topiocqa_dragon_20_generate_70b_test_greedy_0_2600_3438_ret.txt.v2"
    # prediction_file = ckpt_path + "/topiocqa_20_generate_70b_test_greedy_0_2600_3438_ret.txt.v2.reverse_ctxs"
    # prediction_file = ckpt_path + "/topiocqa_20_generate_70b_test_greedy_0_2600_3438_ret.txt.v2.swing_ctxs"
    # prediction_file = ckpt_path + "/topiocqa_20_generate_70b_test_greedy_0_2600_3438_ret.txt.v2.shuffle_ctxs"
    # prediction_file = ckpt_path + "/topiocqa_12_generate_70b_test_greedy_0_2600_3438_ret.txt.v2"
    # prediction_file = ckpt_path + "/topiocqa_30_generate_70b_test_greedy_0_2600_3438_ret.txt.v2"
    # prediction_file = "/lustre/fsw/adlr/adlr-nlp/zihanl/datasets/foundational-qa/multi-turn-qa/topiocqa/chatgpt_3.5_turbo_gen_topiocqa.txt"
    # prediction_file = "/lustre/fsw/adlr/adlr-nlp/zihanl/datasets/foundational-qa/multi-turn-qa/topiocqa/gpt_4_topiocqa_final.txt"
    ...
    ## hybriddial
    prediction_file = ckpt_path + "/hybriddial_{}_generate_70b_test_greedy_0_1200_3438_ret.txt.v2".format(n_ctx)
    # prediction_file = "/lustre/fsw/adlr/adlr-nlp/zihanl/datasets/foundational-qa/multi-turn-qa/HybridDial/gpt_4_hybriddial.txt"
    # prediction_file = "/lustre/fsw/adlr/adlr-nlp/zihanl/datasets/foundational-qa/multi-turn-qa/HybridDial/chatgpt_3.5_turbo_gen_hybriddial.txt"
    ...
    ## inscit
    prediction_file = ckpt_path + "/inscit_20_generate_70b_test_greedy_0_550_3438_ret.txt.v2"
    # prediction_file = ckpt_path + "/inscit_dragon_20_generate_70b_test_greedy_0_550_3438_ret.txt.v2"
    # prediction_file = ckpt_path + "/inscit_20_generate_70b_test_greedy_0_550_3438_ret.txt.v2.reverse_ctxs"
    # prediction_file = ckpt_path + "/inscit_20_generate_70b_test_greedy_0_550_3438_ret.txt.v2.swing_ctxs"
    # prediction_file = ckpt_path + "/inscit_20_generate_70b_test_greedy_0_550_3438_ret.txt.v2.shuffle_ctxs"
    # prediction_file = ckpt_path + "/inscit_12_generate_70b_test_greedy_0_550_3438_ret.txt.v2"
    # prediction_file = ckpt_path + "/inscit_30_generate_70b_test_greedy_0_550_3438_ret.txt.v2"
    # prediction_file = "/lustre/fsw/adlr/adlr-nlp/zihanl/datasets/foundational-qa/multi-turn-qa/inscit/gpt_4_inscit_final.txt"
    # prediction_file = "/lustre/fsw/adlr/adlr-nlp/zihanl/datasets/foundational-qa/multi-turn-qa/inscit/chatgpt_3.5_turbo_gen_inscit.txt"
    ...
    # ## llmware
    # prediction_file1 = ckpt_path + "/llmware_{}_generate_70b_test_greedy_0_500_3458_ret.txt.v2".format(n_ctx)
    # prediction_file2 = ckpt_path + "/llmware_unanswerable_{}_generate_70b_test_greedy_0_500_3458_ret.txt.v2".format(n_ctx)
    # ground_truth_file = "/lustre/fsw/adlr/adlr-nlp/zihanl/datasets/foundational-qa/single-turn-qa/llmware/test.json"
    # print("-"*80)
    # print(prediction_file1)
    # print(prediction_file2)
    # print(ground_truth_file)
    # evaluate_f1_llmware(ground_truth_file, prediction_file1, prediction_file2)
