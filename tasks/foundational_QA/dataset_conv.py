...
from reformat_prompt import reformat_prompt_for_malachite_sawfly
from reformat_prompt import reformat_prompt_for_neat_spoonbill
...
def get_fewshot_file_dict(fewshot_files):

    ## type(fewshot_files) == list
    fewshot_file_dict = {}
    assert len(fewshot_files) % 2 == 0
    num_datasets = int(len(fewshot_files) / 2)

    for idx in range(num_datasets):
        fewshot_file_dict[fewshot_files[2*idx]] = fewshot_files[2*idx+1]

    return fewshot_file_dict
...
class FtDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, name, indexed_dataset, max_seq_length, 
        max_seq_length_dec=0, fewshot_list=None, fixed_shot=False, model_name=None):
...
def build_normal_training_sample_v2(sample,
    ...
    if fixed_shot:
        final_fewshot_list = fewshot_list
    else:
        if fewshot_list:
            total_num_fs_samples = len(fewshot_list)
            # get a random number of few-shot samples used
            # for _ in range(3):
            for _ in range(2):
                # increase the possibility of getting 0 fs_sample to 0.36
                final_num_fs_samples = random.randint(0, total_num_fs_samples)
                if final_num_fs_samples == 0: break

            if final_num_fs_samples > 0:
                # randomly select samples from the fewshot_list
                final_idx_list = random.sample(range(0, total_num_fs_samples), final_num_fs_samples)
                final_fewshot_list = [fewshot_list[idx] for idx in final_idx_list]
            else:
                final_fewshot_list = None
        else:
            final_fewshot_list = None

    if final_fewshot_list:
        input_tokens = reformat_prompt_with_fewshot_samples(query, neighbours, dataset_name, ft_neighbours, final_fewshot_list, len(output_tokens), tokenizer, max_seq_length)
    else:
        if model_name == "malachite_sawfly":
            input_tokens = reformat_prompt_for_malachite_sawfly(query, neighbours, dataset_name, ft_neighbours, len(output_tokens), tokenizer, max_seq_length)
        elif model_name == "neat_spoonbill":
